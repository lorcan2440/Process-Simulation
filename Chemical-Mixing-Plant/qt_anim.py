...
class CustomMainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("CSTR Controller")

        # Create FRAME_A
        self.FRAME_A = QtWidgets.QFrame(self)
        self.FRAME_A.setStyleSheet(
            'QWidget { background-color: ' + f'{QtGui.QColor(210, 210, 235, 255).name()}' + '}')
        ...
        myDataLoop = threading.Thread(name='myDataLoop', target=dataSendLoop,
            daemon=True, args=(self.addData_callbackFunc,))
        myDataLoop.start()
...
def dataSendLoop(addData_callbackFunc):
    ...
    while(True):
        value_to_emit = 12 * np.sin(0.04 * i) + 7 * np.cos(0.11 * i - 0.05) + np.random.normal(0, 1, 1)
        mySrc.data_signal.emit(value_to_emit)  # <- Here you emit a signal!
        i += 1
        time.sleep(0.2)
