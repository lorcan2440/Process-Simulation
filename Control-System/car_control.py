...
class Car:
    ...
    MEASURE_NOISE_STD_DEV = 0.1#4.0
    PROCESS_NOISE_STD_DEV = 0.1#0.5
    ...
    def iterate_system(self, slider: Slider):
        ...
        u = self.pid_controller(Kp=6, Ki=2, Kd=0.03)
    ...
    def animate(self, frame: int, slider: Slider, ax: plt.Axes, lines: tuple, log: bool = True):
        ...
        for i, (val, label) in enumerate(zip(
                [self.v_sp[-1], self.v_true[-1], self.v_meas[-1], self.v_est[-1]],
                ['$v_{sp}$', '$v_{true}$', '$v_{meas}$', '$v_{est}$'])):
            val_text = f'{label} = {val:.2f}'
            leg.get_texts()[i].set_text(val_text)
